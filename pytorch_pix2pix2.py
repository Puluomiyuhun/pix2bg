...
parser.add_argument('--train_epoch', type=int, default=80, help='number of train epochs')
parser.add_argument('--lrD', type=float, default=0.0005, help='learning rate, default=0.0002')
parser.add_argument('--lrG', type=float, default=0.0005, help='learning rate, default=0.0002')
parser.add_argument('--BCE_lambda', type=float, default=1, help='lambda for L1 loss')
parser.add_argument('--L1_lambda', type=float, default=10, help='lambda for L1 loss')
...
if not os.path.isdir(root):
    os.mkdir(root)
if not os.path.isdir(root + 'Fixed_results2'):
    os.mkdir(root + 'Fixed_results2')
...
fixed_x_ = test[:,:,:, 0:256]
fixed_y_ = test[:,:,:, 256:512]
fixed_z_ = test[:,:,:, 512:768]
if img_size != opt.input_size:
    ...
    fixed_z_ = util.imgs_resize(fixed_z_, opt.input_size)
...
for epoch in range(opt.train_epoch):
    ...
    for x_, _ in train_loader:
        ...
        #G_result = x_ * z_ + G_result * (1 - z_)
        D_result = D(x_, G_result).squeeze()
        #G_train_loss = BCE_loss(D_result, Variable(torch.ones(D_result.size()).cuda())) + opt.L1_lambda * L1_loss(G_result * z_, y_ * z_)
        G_train_loss = opt.BCE_lambda * BCE_loss(D_result, Variable(torch.ones(D_result.size()).cuda())) + opt.L1_lambda * L1_loss(G_result, y_)
    ...
    if epoch % 4 == 0 and epoch > 0:
        torch.save(G.state_dict(), root + 'Fixed_results2/' + model + 'generator_param_' + str(epoch) + '.pkl')
        torch.save(D.state_dict(), root + 'Fixed_results2/' + model + 'discriminator_param_' + str(epoch) + '.pkl')
    ...
    fixed_p = root + 'Fixed_results2/' + model + str(epoch + 1) + '.png'
    util.show_result(G, Variable(fixed_x_.cuda(), volatile=True), fixed_y_, Variable(fixed_z_.cuda(), volatile=True), (epoch+1), save=True, path=fixed_p)
...
torch.save(G.state_dict(), root + 'Fixed_results2/' + model + 'generator_param_final.pkl')
torch.save(D.state_dict(), root + 'Fixed_results2/' + model + 'discriminator_param_final.pkl')
