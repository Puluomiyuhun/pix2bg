...
def show_result(G, x_, y_, z_,  num_epoch, show = False, save = False, path = 'result.png'):
    # G.eval()
    x_2 = torch.cat((x_, z_),dim = 1)
    test_images = G(x_2)
    test_images = x_ * z_ + test_images * (1 - z_)

    size_figure_grid = 3
    fig, ax = plt.subplots(x_.size()[0], size_figure_grid, figsize=(20, 20))
...
def show_result2(G, x_, y_, z_,  num_epoch, show = False, save = False, path = 'result.png'):
    # G.eval()
    x_2 = torch.cat((x_, z_),dim = 1)
    test_images = G(x_2)
    tmp = test_images
    ...
    for i in range(x_.size()[0]):
        ...
        ax[i, 1].imshow((test_images[i].cpu().data.numpy().transpose(1, 2, 0) + 1) / 2)
        ax[i, 2].cla()
        ax[i, 2].imshow((tmp[i].cpu().data.numpy().transpose(1, 2, 0) + 1) / 2)
...
def imgs_resize(imgs, resize_scale = 286):
    ...
    for i in range(imgs.size()[0]):
        img = np.array(Image.fromarray(imgs[i]).resize((resize_scale, resize_scale)))
